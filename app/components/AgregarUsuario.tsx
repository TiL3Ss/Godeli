// app/components/AgregarUsuario.tsx
'use client';
...
const AgregarUsuario: React.FC<AgregarUsuarioProps> = ({ 
  onClose, 
  onUsuarioCreado 
}) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      onUsuarioCreado(data.data);
      onClose();

    } catch (error) {
      setMensaje({ 
        tipo: 'error', 
        texto: error instanceof Error ? error.message : 'Error al crear usuario' 
      });
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-gray-600">Crear una nueva cuenta de usuario</p>
              </div>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="flex gap-3 pt-6">
              ...
              <button
                type="submit"
                disabled={cargando}
                className="flex-1 px-6 py-3.5 bg-gradient-to-r from-green-500 to-blue-600 hover:from-green-600 hover:to-blue-700 text-white rounded-2xl font-semibold transition-all duration-200 disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2 shadow-lg"
              >
                {cargando ? (
                  <>
